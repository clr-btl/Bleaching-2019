...
ggarrange(ggplot(df_plot_temp%>%filter(Variable=="AHS")) +
  ...
  #scale_y_discrete(expand = c(0, 0)) +
    ...
    aes(x = 5.05, y = fct_rev(factor(Depth)), label = letters),
    inherit.aes = FALSE,
    hjust = 0,
    size = 2.5
  ) +
  ...
  scale_x_continuous(expand = c(0.000, 0.000)) +#limits = c(0, 100)
    ...
    aes(x = 2.525, y = fct_rev(factor(Depth)), label = letters),
    inherit.aes = FALSE,
    hjust = 0,
    size = 2.5
  ) +
  ...
  labs(x = '', y = ""),nrow=2,align="hv")

ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Temperature/DHW_MDTF_LTER.pdf",width = 6, height = 3)
```
...
plot=ggplot(data=sst_df)+
  ...
  geom_line(aes(x=Date, y=DHW_scaled),color="white",size=2)+
    ...
    labs(y = "SST CRW temperature (°C)",x='',title=site,fill="DHW")+
    scale_y_continuous(limits=c(25,31.5),oob = scales::rescale_none,expand=c(0,0),
      sec.axis = dup_axis(breaks = rescaledVals$breaks, labels = rescaledVals$labels, name = "DHW (°C weeks)"))+ #rescaledVals$breaks
...
ggarrange(plotlist=plot_list,labels=LETTERS[1:6],ncol=3,nrow=2,align='hv')
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Temperature/SST_DHW_arranged_goodone.pdf",width =12, height = 5)

### Version 2 sans labels 
plot_list=list()
for (site in c("Haapiti","Gendron","E2B","Temae","Afareitu","Maatea")){
  sst_df=sst%>%filter(Site==site)
  rescaledVals=calc_fudge_axis(sst_df$SST, sst_df$DHW, fixed_ylim2 = common_ylim2)
...
dhw1_v2=ggarrange(plotlist=plot_list,labels=c("Haapiti","Gendron","E2B","Temae","Afareitu","Maatea"),ncol=3,nrow=2,align='hv',font.label = list(size=11))
dhw1_v2
```
...
plot=ggplot(data=sst_df)+
  ...
  geom_line(aes(x=Date, y=DHW_scaled),color="white",size=2)+
    ...
    labs(y = "SST CRW temperature (°C)",x='',title=site,fill="DHW")+
    scale_y_continuous(limits=c(25,31.5),oob = scales::rescale_none,expand=c(0,0),
      sec.axis = dup_axis(breaks = rescaledVals$breaks, labels = rescaledVals$labels, name = bquote(DHW["0°C"] ~ "(°C.weeks)")))+ #rescaledVals$breaks
  theme(axis.title.y.left=element_text(size=9),axis.ticks.y.right = element_line(color=color_second_y),
        axis.text.y.right = element_text(color=color_second_y),
        text=element_text(family="sans"),
        axis.line.y.right = element_line(color=color_second_y),
...
ggarrange(plotlist=plot_list,labels=LETTERS[1:6],ncol=3,nrow=2,align='hv')
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Temperature/SST_DHW0°C_arranged_goodone.pdf",width =12, height = 5)
...
dhw0_v2=ggarrange(plotlist=plot_list,labels=c("Haapiti","Gendron","E2B","Temae","Afareitu","Maatea"),ncol=3,nrow=2,align='hv',font.label = list(size=11))
dhw0_v2

ggarrange(NULL,dhw1_v2,NULL,dhw0_v2,labels=c("A - DHW cutoff 1°C","","B - DHW cutoff 0°C",""),nrow=4,align="hv",heights=c(0.05,1,0.05,1),hjust=-0.05,font.label = list(color="grey30"))
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Temperature/Arranged_DHW1_DHW0_v2.pdf",width =11, height = 9)
```
...
```{r Good version plot for Moorea map}
library(forcats)



plot_list=list()
for (Site in unique(overall_counts_site$site)){
df_plot=overall_counts_site%>%filter(site==Site)
...
plot=ggplot(df_plot, aes(x = percent, y = fct_rev(depth), fill = status)) +
  geom_col() +
  geom_text(
    data = labels_n,
    aes(x = x, y = depth, label = label),
  ...
  theme(
    strip.background = element_blank(),
    strip.placement = "outside",
    strip.text.y.left = element_text( size = 10, angle = 0, hjust = 1,face="bold"),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.border = element_rect(color = "black", size = 0.4),
    axis.ticks = element_line(size=0.4),
...
ggarrange(plotlist=plot_list,nrow=6,align="hv")

ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Proportion_bleaching_bysite_depth_for_map1.pdf",width = 4.7, height = 5.2)
```
...
plot=ggplot(df_plot, aes(x = percent, y = fct_rev(depth), fill = status)) +
  ...
  theme(
    strip.background = element_blank(),
    strip.placement = "outside",
    strip.text.y.left = element_blank(),  # ← on masque le côté gauche
strip.text.y.right = element_text(size = 9, angle = 0, hjust = 0, face = "bold"),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.border = element_rect(color = "black", size = 0.4),
     axis.text.y.right = element_text(size = 8),
  axis.ticks.y.right = element_line(size = 0.4),
...
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Proportion_bleaching_bysite_depth_for_map2.pdf",width = 5, height = 7)
```
...
ggplot(overall_counts_genus, aes(x = genus_label, y = percent, fill = status)) +
  ...
  theme(
    ...
    legend.text = element_text(size = 7, margin = margin(t = 0, unit = "lines")),
    legend.key.size = unit(0.3, "cm"),
    legend.spacing.y = unit(0, "pt"),
    legend.margin = margin(0, 0, 0, 0),
    legend.direction = "horizontal",
    legend.position = "top",
    legend.justification.top = c(-0.02, 0.6)
  ) +
  scale_fill_manual(name = "Status", values = c("thistle4", "thistle2", "snow2"), drop = FALSE)
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Bleaching_genus_pooleddepth.pdf",width =6, height = 4.5)
...
plot_pooled_depth=ggplot(overall_counts_genus, aes(x = genus_label, y = percent, fill = status)) +
  geom_bar(stat = "identity", alpha = 0.7) +
  labs(
    title = "Pooled depth",
    x = "",
...
plot_genus_susceptibility_depth=list(plot_pooled_depth)
for (depth in c("5m","12m","20m")){
  df=overall_counts_genus_depth_full[overall_counts_genus_depth_full$depth==depth,]

  
  plot=ggplot(df, aes(x = genus_label, y = percent, fill = status)) +
  geom_bar(stat = "identity", alpha = 0.7) +
  labs(
    title = depth,
    x = "",
  ...
  plot_genus_susceptibility_depth=append(plot_genus_susceptibility_depth,list(plot))
}

ggarrange(plotlist=plot_genus_susceptibility_depth,labels=c("A","B","C","D"),nrow=2,ncol=2,align='hv')
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Bleaching_genus_by_depth.pdf",width =12, height = 8)
...
#####
predictions%>% 
          ggplot(aes(x = Depth, y = predicted,color=Depth)) +   
            geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width=0.15, size = 0.5,position=position_dodge(.7),alpha=0.9,show.legend=F) +
            geom_bar(aes(fill=Depth),alpha=0.3,position=position_dodge(.7),stat="identity",width=0.7,color=NA)+
            ...
            labs(y="Probability of bleaching")+
            theme(axis.title.x=element_blank(),legend.position="none",plot.title=element_text(face="italic",size=12,hjust=-0.03),legend.title=element_blank(),legend.text = element_text(size = 7,margin = margin(t = 0, unit = "lines")),legend.background=element_blank(),legend.key.size = unit(0.3, "cm"),legend.spacing.y = unit(0, "pt"),legend.margin = margin(0, 0, 0, 0))

#####
predictions$depth=factor(predictions$Depth,levels=c("5m","12m","20m"),labels=c(5,12,20))
predictions$depth=as.numeric(as.character(predictions$depth))

bleaching_proportion=bleaching_long %>%
  group_by(site, quadrat, depth) %>%
  dplyr::summarise(
    n_bleached = sum(n[Bleached == 1], na.rm = TRUE),
    n_total = sum(n, na.rm = TRUE),
    prop_bleached = n_bleached / n_total
  ) %>%
  ungroup()
bleaching_proportion$Depth=factor(bleaching_proportion$depth,levels=c("5m","12m","20m"),labels=c(5,12,20))
bleaching_proportion$Depth=as.numeric(as.character(bleaching_proportion$Depth))

ggplot() +
  geom_jitter(data = bleaching_proportion,aes(y = Depth, x = prop_bleached, color = depth, fill = depth),size = 2, alpha = 0.1, height = 1) +
  geom_errorbarh(data = predictions,aes(y = depth, x = predicted, xmin = conf.low, xmax = conf.high,color=Depth),height = 0.8,size = 1,show.legend =FALSE,width=0.2,height=c(0.4,0.4,0.4)) +
  #geom_errorbarh(data = predictions,aes(y = depth, x = predicted, xmin = conf.low,xmax=conf.high,group=1),height = 0.4, size = 1, show.legend = FALSE,width=0.2,height=0.4,color="black") +
  geom_point(data = predictions,aes(y = depth, x = predicted, color = Depth),shape = 21, fill = "white", stroke = 2, size = 2) +
  #geom_point(data = predictions,aes(y = depth, x = predicted),shape = 21, fill = "white", stroke = 2, size = 2,color="black") +
  #scale_color_manual(values = rev(c("#09283CFF", "mediumpurple1", "skyblue1"))) +
  #scale_fill_manual(values = rev(c("#09283CFF", "mediumpurple1", "skyblue1"))) +
  
  scale_color_manual(
  values = c("5m" = "skyblue1", "12m" = "#259FAF", "20m" = "#044553")
) +
scale_fill_manual(
  values = c("5m" = "skyblue1", "12m" = "#259FAF", "20m" = "#044553")
)+
  ...
  theme_classic() +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 1.02),position="top") +
  scale_y_reverse(breaks = c(2,5, 12, 20), limits = c(22, 2),expand=c(0,0)) +  # ← profondeur inversée
  labs(x = "Proportion bleaching", y = "Depth (m)") +
...
#ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Binomial/Predictions_genus_random_pale_as_bleached3.pdf",width =5, height = 2.7) 
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Binomial/Predictions_genus_random_pale_as_bleached3.pdf",width =4, height = 2) 


#####
predictions$depth=factor(predictions$Depth,levels=c("5m","12m","20m"),labels=c("5","12","20"))
bleaching_proportion$Depth=factor(bleaching_proportion$depth,levels=c("5m","12m","20m"),labels=c("5","12","20"))
predictions$depth=factor(predictions$depth,levels=c("20","12","5"))
bleaching_proportion$Depth=factor(bleaching_proportion$Depth,levels=c("20","12","5"))

ggplot() +
  geom_jitter(data = bleaching_proportion,aes(y = Depth, x = prop_bleached, color=Depth),size = 1, alpha = 0.05, height = 0.2) +
  geom_errorbar(data = predictions,aes(y = depth, x = predicted, xmin = conf.low, xmax = conf.high,color=depth),size = 0.7,show.legend =FALSE,width=0.2) +
  geom_col(data = predictions,aes(y = depth, x = predicted, fill = depth),stat="identity",width = 0.7,color = NA,alpha=0.2) +
   scale_color_manual(values = c("#09283CFF", "mediumpurple2","skyblue1")) +
   scale_fill_manual(values = c("#09283CFF", "mediumpurple2","skyblue1"))+
  theme_classic() +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 1.02),position="top") +
...
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Binomial/Predictions_genus_random_pale_as_bleached3.pdf",width =3.5, height = 2) 
```
...
#####
width_bars=0.8
plot_list_model_genus=list()
for (genus in levels(predictions$Genus)){
  predictions_sub=predictions[predictions$Genus==genus,]
  plot=predictions_sub %>% 
          ggplot(aes(x = Depth, y = predicted,color=Depth)) +   
            geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width=0.15, size = 0.5,position=position_dodge(width_bars),alpha=0.9,show.legend=F) +
            geom_bar(aes(fill=Depth),alpha=0.3,position=position_dodge(width_bars),stat="identity",width=width_bars,color=NA)+
            scale_color_manual(values = rev(c("#09283CFF", "mediumpurple2","skyblue1"))) +
            scale_fill_manual(values = rev(c("#09283CFF", "mediumpurple2","skyblue1")))+
            #scale_color_manual(values = c("skyblue1", "#259FAF", "#044553")) +
            #scale_fill_manual(values = c("skyblue1", "#259FAF", "#044553")) +
            theme_classic()+
            scale_y_continuous(expand=c(0,0),limits=c(0,1.01))+
            scale_x_discrete(expand=c(0.2,0.2))+
            labs(y="Probability of bleaching",title="")+
            theme(axis.title.x=element_blank(),
                  legend.position="none",
                  plot.title=element_text(face="italic",size=10,hjust=-0.03))+
    theme(axis.title.y=element_text(size=7),axis.text.x=element_text(size=7),axis.text.y=element_text(size=6)) #to add for plotting in 1 row + plot.title size = 12
  
  plot_list_model_genus=append(plot_list_model_genus,list(plot))

}



ggarrange(plotlist=plot_list_model_genus,labels=c("Acropora","Phymastrea","Montipora","Millepora","Leptastrea","Pocillopora","Porites","Pavona"),align="hv",ncol=4,nrow=2,font.label=list(size=9,face="italic"),hjust=-0.2) #LETTERS[1:length(unique(predictions$Genus))]
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Binomial/Predictions_genus_fixed_arranged_pale_as_healthyv2.pdf",width =7, height = 3.5)
```

```{r Good version plot - barplot predictions}
...
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Binomial/Predictions_genus_fixed_arranged_pale_as_bleached_barplotv4.pdf",width =5, height = 5)
```

```{r Plot contrasts model -  differences by genra}
emm=emmeans(mod_bleaching2,pairwise~genus|depth,type="response",adjust="tukey",bias.adjust=T,sigma=total_sd)
matrix_contrasts_all=pwpm(regrid(emm,bias.adjust=T,sigma=total_sd), by = c("depth"), means = FALSE,adjust="tukey",bias.adjust=T,sigma=total_sd) #regrid(emm,bias.adjust=T,sigma=total_sd)

depths=c("5m", "12m", "20m")
genus_list=c("Pocillopora","Montipora","Acropora","Phymastrea","Pavona","Porites","Leptastrea","Millepora")


plot_list=list()
for (depth in depths) {
    
    combination_name=paste("depth =", depth)
    ...
    long_p_values=as.data.frame(p.matx, check.names = FALSE) %>%
...
arranged_dead=ggarrange(plotlist=plot_list,labels=paste0(LETTERS[1:12]," - ",depths),ncol=3,nrow=1,common.legend=T,legend="none",font.label=list(size=14),hjust=-0.2)
annotation_dead=ggplot() +
  annotate(
    "text",
    label = "B L E A C H I N G",
    x = 0.5,
    y = 0.5,
    size = 7,
  ...
  theme_void() +
  theme(
    plot.background = element_rect(fill = "tan",color=NA),axis.line.x = element_blank(),axis.line.y = element_blank()
  )

arranged_dead=ggarrange(annotation_dead,arranged_dead,widths=c(0.02,0.98))
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Binomial/Emmeans_contrats_bleaching_tukey_differences_genus.pdf",width = 16, height = 5)
```
...
genus_list=rev(c("Acropora","Leptastrea","Millepora","Montipora","Pavona","Phymastrea","Pocillopora","Porites"))
  ...
  plot_genus_ave_depth=combined %>%
    ...
    geom_tile(aes(alpha = ifelse(is_upper, 1, 0)), fill = "white",color=NA) +
    #geom_text(aes(label = ifelse(is_upper, p_values_text, round(Differences,2)),fontface = ifelse(p_values<0.05, 2, 1)),size=3) +
      # Text for upper triangle (p-values), small text
    geom_text(data = ~ .x %>% filter(is_upper), aes(label = p_values_text, fontface = ifelse(p_values_text < 0.05, 2, 1)),size = 2.6)+
  # Text for lower triangle (Differences), larger text
    geom_text(data = ~ .x %>% filter(!is_upper & !is_diag),aes(label = round(Differences, 2),fontface = ifelse(p_values_text < 0.05, 2, 1)),size = 3) +
    scale_fill_gradient2(name = "Differences", low = "darkmagenta", mid = "white", high = "darkorange", midpoint = 0, limits = c(-1, 1)) +
    scale_alpha_identity() +
    coord_equal(expand = FALSE)+
    labs(x = "",y = "",title=" ") +
...
arranged_dead=ggarrange(annotation_dead,plot_genus_ave_depth,widths=c(0.06,0.94))
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Binomial/Emmeans_contrats_bleaching_tukey_differences_genus_averaged_depth.pdf",width = 5, height = 4.9)
```

```{r Plot contrasts model - differences by depth}
emm=emmeans(mod_bleaching2,pairwise~depth|genus,type="response",adjust="tukey",bias.adjust=T,sigma=total_sd)
...
for (genus in genus_list) {
  ...
  plot=combined %>%
    ...
    #geom_text(aes(label = ifelse(is_upper, p_values_text, round(Differences,2)),fontface = ifelse(p_values<0.05, 2, 1)),size=3.5) +
      # Text for upper triangle (p-values), small text
    geom_text(data = ~ .x %>% filter(is_upper), aes(label = p_values_text, fontface = ifelse(p_values_text < 0.05, 2, 1)),size = 2.9)+
  # Text for lower triangle (Differences), larger text
    geom_text(data = ~ .x %>% filter(!is_upper & !is_diag),aes(label = round(Differences, 2),fontface = ifelse(p_values_text < 0.05, 2, 1)),size = 3.4) +
...
arranged_dead=ggarrange(plotlist=plot_list,labels=genus_list,ncol=4,nrow=2,common.legend=T,legend="none",font.label=list(size=14,face="italic"),hjust=-0.2) #paste0(LETTERS[1:12]," - ",genus_list)
ggarrange(NULL,ggarrange(NULL,arranged_dead,nrow=2,heights=c(0.02,0.98)),plot_genus_ave_depth,labels=c("A","","B"),widths=c(0.02,0.65,0.33),ncol=3)
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Bleaching/Binomial/Emmeans_contrats_bleaching_arranged.pdf",width = 14, height = 4.7)
...
arranged_dead=ggarrange(annotation_dead,arranged_dead,widths=c(0.03,0.97))
...
### % mortality
```{r}
mortality_summary=mortality_coral %>%
  group_by(Depth) %>%
  summarise(
    total_points_corals = sum(n_point_sp),
    dead_points_corals = sum(n_point_sp[State == "Dead"]),  #dead points
    living_points = sum(n_point_sp[State=="Living"]),  #living points
    mortality_percent = (dead_points_corals / total_points_corals) * 100  #% mortality
  ) %>%
  ungroup()


mortality_summary_genus=mortality_coral %>%
  group_by(Depth,Genus) %>%
  summarise(
    total_points_corals = sum(n_point_sp),
    dead_points_corals = sum(n_point_sp[State == "Dead"]),  #dead points
...
ggplot(predictions, aes(x = Depth)) +
  geom_errorbar(
    aes(ymin = -conf.low * 100, ymax = -conf.high * 100, color = Depth),
    width = 0.15, size = 0.5,
    position = position_dodge(width = 0.7),alpha=0.9,
    show.legend = FALSE
  ) +
  geom_bar(
    aes(y = -predicted * 100, fill = Depth),
    stat = "identity",
    position = position_dodge(width = 0.7),
    width = 0.7, color = NA,alpha=0.3
  ) +
  scale_color_manual(values = rev(c("black", "#8A2BE2", "#0096C7"))) +
  scale_fill_manual(values = rev(c("#09283CFF", "mediumpurple1", "skyblue1"))) +
  theme_classic() +
  scale_y_continuous(expand = c(0, 0), limits = c(-51,0), breaks = seq(-60, 0, by = 10)) +
  ...
  theme(axis.text.x=element_text(size=9),
    ...
    plot.title = element_text(face = "italic", size = 12, hjust = -0.03),
    legend.title = element_blank(),
    legend.text = element_text(size = 7, margin = margin(t = 0, unit = "lines")),
...
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Model/Cover loss/Predictions_relative_cover_loss.pdf",width =3, height = 2.5)


#####
predictions$depth=factor(predictions$Depth,levels=c("5m","12m","20m"),labels=c("5","12","20"))
cover_final$depth=factor(cover_final$depth,levels=c("20","12","5"))

ggplot() +
  geom_jitter(data = cover_final,aes(y = depth, x = Cover_loss_relative, color=depth),size = 1, alpha = 0.07, height = 0.2) +
  geom_errorbar(data = predictions,aes(y = depth, x = predicted, xmin = conf.low, xmax = conf.high,color=depth),size = 0.7,show.legend =FALSE,width=0.2) +
  geom_col(data = predictions,aes(y = depth, x = predicted, fill = depth),width = 0.7,color = NA,alpha=0.2) +
   scale_color_manual(values = c("#09283CFF", "mediumpurple2","skyblue1")) +
   scale_fill_manual(values = rev(c("#09283CFF", "mediumpurple2","skyblue1")))+
  theme_classic() +
  scale_x_continuous(expand = c(0, 0), limits = c(0,1.02),position="top",breaks=seq(0,1,by=0.25)) +
  labs(x = "Relative coral cover loss", y = "Depth (m)") +
  theme(axis.title.x = element_text(size = 9),
    axis.title.y = element_text(size = 9,angle=90),
    legend.position = "none", axis.text=element_text(size=8)
  )
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Model/Cover loss/Predictions_relative_cover_loss_v2.pdf",width =3.5, height = 2) 
```
...
for (genus in levels(predictions$Genus)){
  ...
  scale_fill_manual(values = rev(c("#09283CFF", "mediumpurple1", "skyblue1"))) +
  theme_classic() +
  scale_y_continuous(expand = c(0, 0), limits = c(-105,0), breaks = seq(-100, 0, by = 20)) +
  scale_x_discrete(expand = c(0.2, 0.2),position="top") +
  labs(y = "Change in coral cover (%)",title=genus) +
  guides(fill="none",color="none")+
  theme(axis.text.x=element_text(size=9),
         axis.title.y=element_text(size=9),
        axis.text.y=element_text(size=8),
    axis.title.x = element_blank(),
    legend.position = c(0.2,0.95),
    plot.title = element_text(face = "italic", size = 11, hjust = -0.03),
    legend.title = element_blank(),
    legend.text = element_text(size = 7, margin = margin(t = 0, unit = "lines")),
    legend.background = element_blank(),
    legend.key.size = unit(0.3, "cm"),
...
ggarrange(plotlist=plot_list_model_genus,labels=LETTERS[1:length(unique(predictions$Genus))],align="hv",ncol=3)
#ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Model/Cover loss/Predictions_relative_cover_loss_by_genus.pdf",width =6.5, height = 2.5)

#####
predictions$depth=factor(predictions$Depth,levels=c("5m","12m","20m"),labels=c("5","12","20"))
predictions$depth=factor(predictions$depth,levels=c("20","12","5"))
cover_final_genus$depth=factor(cover_final_genus$Depth,levels=c("5m","12m","20m"),labels=c("5","12","20"))
cover_final_genus$depth=factor(cover_final_genus$depth,levels=c("20","12","5"))

plot_list_model_genus=list()
for (genus in levels(predictions$Genus)){
  predictions_sub=predictions[predictions$Genus==genus,]
  cover_final_genus_sub=cover_final_genus[cover_final_genus$Species==genus,]

plot=ggplot() +
  geom_jitter(data = cover_final_genus_sub,aes(y = depth, x = Cover_loss_relative, color=depth),size = 1, alpha = 0.1, height = 0.2) +
  geom_errorbar(data = predictions_sub,aes(y = depth, x = predicted, xmin = conf.low, xmax = conf.high,color=depth),size = 0.7,show.legend =FALSE,width=0.2) +
  #geom_point(data = predictions_sub,aes(x = Depth, y = predicted, color = Depth),shape = 21, fill = "white", stroke = 2, size = 2) +
  geom_col(data = predictions_sub,aes(y = depth, x = predicted, fill = depth),stat="identity",width = 0.7,color = NA,alpha=0.2) +
  #scale_color_manual(values = rev(c("skyblue1", "#259FAF", "#044553"))) +
  #scale_fill_manual(values = rev(c("skyblue1", "#259FAF", "#044553"))) +
   scale_color_manual(values = c("#09283CFF", "mediumpurple2","skyblue1")) +
   scale_fill_manual(values = c("#09283CFF", "mediumpurple2","skyblue1"))+
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(0, 1.02),position="top") +
  #scale_y_reverse(breaks = c(2,5, 12, 20), limits = c(22, 2),expand=c(0,0)) +  # ← profondeur inversée
  labs(x = "Relative coral cover loss", y = "Depth (m)",title=genus) +
  theme(axis.title.x = element_text(size = 9),
    axis.title.y = element_text(size = 9),
    legend.position = "none", axis.text=element_text(size=8),
    plot.margin = margin(0, 10, 10, 10),
  plot.title = element_textbox_simple(size = 11,face="italic",
      color = "seashell4", fill = "seashell2",box.color = NA,
      halign = 0.05, linetype = 1, width = unit(1.33, "npc"), valign=1.5,
      #padding = margin(2, 0, 1, 0), margin = margin(3, 3, 5, 3),
      padding = margin(2, 0, 1, 0),  margin = margin(0, 0, 8, 0))
  )
plot_list_model_genus=append(plot_list_model_genus,list(plot))
}
ggarrange(plotlist=plot_list_model_genus,align="hv",ncol=2,nrow=1)#,labels=levels(predictions$Genus),font.label = list(face="bold.italic",size=10),hjust=-0.2)
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Model/Cover loss/Predictions_relative_cover_loss_by_genusv3.pdf",width =6, height = 2)
```
...
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Model/Cover loss/Predictions_relative_cover_loss_by_genusv4.pdf",width =5, height = 1.4)
```

```{r Plot contrasts model -  differences by genra}
emm=emmeans(mod_cover_loss_relative_genus,pairwise~Species|Depth,type="response",adjust="tukey",bias.adjust=T,sigma=total_sd)
matrix_contrasts_all=pwpm(regrid(emm,bias.adjust=T,sigma=total_sd), by = c("Depth"), means = FALSE,adjust="tukey",bias.adjust=T,sigma=total_sd) #regrid(emm,bias.adjust=T,sigma=total_sd)

depths=c("5m", "12m", "20m")
genus_list=c("Pocillopora","Montipora","Acropora")
...
for (depth in depths) {
  ...
  plot=combined %>%
    ...
    geom_tile(aes(alpha = ifelse(is_diag, 1, 0)), fill ="#DEDEDE") +
    geom_tile(aes(alpha = ifelse(is_upper, 1, 0)), fill = "white",color=NA) +
    geom_text(aes(label = ifelse(is_upper, p_values_text, round(Differences,2)),fontface = ifelse(p_values<0.05, 2, 1)),size=4) +
    scale_fill_gradient2(name = "Differences", low = "darkmagenta", mid = "white", high = "darkorange", midpoint = 0, limits = c(-0.8, 0.8)) +
    scale_alpha_identity() +
    coord_equal(expand = FALSE)+
    labs(x = "",y = "",title=" ") +
    theme_bw() +
...
annotation_dead=ggplot() +
  ...
  theme_void() +
  theme(
    plot.background = element_rect(fill = "darkkhaki",color=NA),axis.line.x = element_blank(),axis.line.y = element_blank()
  )

arranged_dead=ggarrange(annotation_dead,arranged_dead,widths=c(0.04,0.96))
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Model/Cover loss/Emmeans_contrats_dead_tukey_differences_genus.pdf",width = 9, height = 3.1)
```
...
for (genus in genus_list) {
  ...
  plot=combined %>%
    ...
    geom_tile(aes(alpha = ifelse(is_upper, 1, 0)), fill = "white",color=NA) +
    geom_text(aes(label = ifelse(is_upper, p_values_text, round(Differences,2)),fontface = ifelse(p_values<0.05, 2, 1)),size=4) +
    scale_fill_gradient2(name = "Differences", low = "darkmagenta", mid = "white", high = "darkorange", midpoint = 0, limits = c(-0.5, 0.5)) +
    scale_alpha_identity() +
    coord_equal(expand = FALSE)+
    labs(x = "",y = "",title=" ") +
    theme_bw() +
    ...
    plot.title = element_text(face = "italic",size=10), legend.direction="horizontal", legend.position="bottom",
    axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),                       
    axis.text.x.bottom = element_text(size=13),axis.text.y = element_text(size=13))#+theme(plot.margin = unit(c(0,0,0,0), "lines"))

    plot_list=append(plot_list, list(plot))
  }

arranged_dead=ggarrange(plotlist=plot_list,labels=genus_list,ncol=2,nrow=1,common.legend=T,legend="none",font.label=list(size=14,face="italic"),hjust=-0.2)
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Model/Cover loss/Emmeans_contrats_dead_tukey_differences_good_one.pdf",width = 6, height = 2.7)
...
predictions_mortality=ggplot() +
  ...
  theme(
    strip.background = element_blank(),
    strip.placement = "outside",
    strip.text.y.left = element_text( size = 12, angle = 0, hjust = 1,face="italic"),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.border = element_rect(color = "grey30", size = 0.7),
    axis.ticks = element_line(size=0.4),
...
for (genus in genus_list) {
  ...
  plot=combined %>%
    ...
    plot.title = element_text(face = "italic",size=10), legend.direction="horizontal", legend.position="bottom",
    axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),                       
    axis.text.x.bottom = element_text(size=10),axis.text.y = element_text(size=10))#+theme(plot.margin = unit(c(0,0,0,0), "lines"))

    plot_list=append(plot_list, list(plot))
  }

arranged_dead=ggarrange(plotlist=plot_list,labels=genus_list,ncol=2,nrow=1,common.legend=T,legend="none",font.label=list(size=12,face="italic"),hjust=-0.2)

ggarrange(predictions_mortality,NULL,ggarrange(NULL,arranged_dead,ncol=2,widths=c(0.1,0.9)),labels=c("A","","B"),nrow=3,heights=c(0.5,0.05,0.7))
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Model/Cover loss/Arranged_predictions_emmeanns.pdf",width = 5.5, height = 4)
...
assemblage=assemblage%>%
  filter(
    !(Site == "Gendron" & Depth == "20m" & Quadrat == 6),
    !(Site == "Maatea"  & Depth == "20m" & Quadrat == 9)
  )
```
...
```{r Plot coral assemblages by depth and status}
palette=c(
  "#009392",  # teal (bleu-vert profond)
  "#39B185",  # vert doux
  "#9CCB86",  # vert amande
...
mortality_coral_compo$Genus=factor(mortality_coral_compo$Genus,levels=rev(c("Acropora","Pocillopora","Montipora","Porites","Pavona","Phymastrea","Leptastrea","Millepora","Others")))

ggplot(mortality_coral_compo, aes(x = percent, y = fct_rev(Depth), fill = Genus)) +
  geom_col(alpha=0.7) +
  scale_fill_manual(name="Taxa",values=palette,drop=F,guide = guide_legend(reverse = TRUE))+
  facet_grid(rows = vars(factor(State)), switch = "y") +
  scale_x_continuous(expand = c(0, 0)) +#limits = c(0, 100)
  scale_y_discrete(expand = c(0, 0)) +
  coord_cartesian(clip = "off",xlim=c(0,100)) +  # ← autorise l’affichage en dehors du panel
  theme_bw() +
  theme(
    strip.background = element_blank(),
    strip.placement = "outside",
    strip.text.y.left = element_text( size = 10, angle = 0, hjust = 1,margin = margin(0,10,0,0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.border = element_rect(color = "black", size = 0.4),
    axis.ticks = element_line(size=0.4),
  ...
  labs(x = "Relative abundance (%)", y = "")
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Coral assemblages/Alive_vs_dead_compositionv3.pdf",width =5, height = 1.9)
```
...
ggplot(nmds_scores, aes(x = NMDS1, y = NMDS2)) +
  ...
  geom_polygon(data = hull_CC_NMDS, aes(x = NMDS1, y = NMDS2, fill = ID), alpha = 0.3, color = NA, show.legend = T) +
  geom_point(aes(color=Depth),size = 1, alpha = 0.2) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        panel.border = element_rect(size = 1),
        legend.title=element_text(size=9),legend.text = element_text(size = 7,margin = margin(t = 0, unit = "lines")),legend.key.size = unit(0.3, "cm"),legend.spacing.y = unit(0, "pt"),legend.margin = margin(0, 0, 0, 0), strip.text = element_textbox(
      size = 12,
      color = "seashell4", fill = "seashell2", box.color = NA,
      halign = 0.5, linetype = 1, r = unit(5, "pt"), width = unit(1, "npc"),
      padding = margin(2, 0, 1, 0), margin = margin(3, 3, 3, 3)
...
#ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Coral assemblages/NMDS_k=2_before-after.pdf",width =8, height = 3.5)
...
ggarrange(nmds_plot,boxplot_shift,ncol=2,widths=c(0.73,0.27),labels=c("A","B"),align="v")
ggsave("~/Documents/PhD/Bleaching 2019/Results/Moorea/Mortality/Coral assemblages/nMDS_arranged.pdf",width =10, height = 3)
